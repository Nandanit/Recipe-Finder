	...
	*/
	...
	public function recommendRecipe(){
		...
		for($rcpCounter=0;$rcpCounter<count($this->recipes);$rcpCounter++){
			...
			list($ingredientAvailableToMakeRecipe, $recipeExpiryDate) =  $this->ingredientsInFridge($ingredientsRequired);	
     ...
     */
	public function ingredientsInFridge($ingredientsRequiredForRecipe){
		$recipeExpireDate = null;
		for($i=0;$i<count($ingredientsRequiredForRecipe);$i++){
			$ingrdntInFrdge = false;		
			$ingredient = $ingredientsRequiredForRecipe[$i];  
	 ...
	 /**
     * parse recipe JSON file and set recipes available 
     * 
     * @param array $recipe 
     * @access public
     */

	public function readRecipeJsonFile($recipeJsonFile){
	...
	public function readFridgeCsvFile($fridgeCsvFile) {
		$fridgCsvHandler = fopen($fridgeCsvFile,"r");
		while(!feof($fridgCsvHandler)){
			list($item, $amount, $unit, $useByDate) = fgetcsv($fridgCsvHandler);
